...
class Viewer3D(object):
    ...
    def viewer_thread(self, qmap, qvo, is_running, is_paused):
        ...
        while not pypangolin.ShouldQuit() and (is_running.value == 1):
            self.viewer_refresh(qmap, qvo, is_paused)
        print('Quitting viewer...')    

    def viewer_init(self, w, h):
        pypangolin.CreateWindowAndBind('Map Viewer', w, h)
        ...
        self.proj = pypangolin.ProjectionMatrix(w, h, viewpoint_f, viewpoint_f, w//2, h//2, 0.1, 5000)
        self.look_view = pypangolin.ModelViewLookAt(viewpoint_x, viewpoint_y, viewpoint_z, 0, 0, 0, 0, -1, 0)
        self.scam = pypangolin.OpenGlRenderState(self.proj, self.look_view)
        ...
        self.dcam.SetHandler(pypangolin.Handler3D(self.scam))

        self.panel = pypangolin.CreatePanel('ui')
        ...
        self.checkboxFollow = pypangolin.VarBool('ui.Follow', value=True, toggle=True)
        self.checkboxCams = pypangolin.VarBool('ui.Draw Cameras', value=True, toggle=True)
        self.checkboxCovisibility = pypangolin.VarBool('ui.Draw Covisibility', value=True, toggle=True)  
        self.checkboxSpanningTree = pypangolin.VarBool('ui.Draw Tree', value=True, toggle=True)                
        self.checkboxGrid = pypangolin.VarBool('ui.Grid', value=True, toggle=True)           
        self.checkboxPause = pypangolin.VarBool('ui.Pause', value=False, toggle=True)             
        self.int_slider = pypangolin.VarInt('ui.Point Size', value=kDefaultPointSize, min=1, max=10)  

        self.pointSize = self.int_slider.Get()

        self.Twc = pypangolin.OpenGlMatrix()
        ...
        if self.map_state is not None:
            ...
            if len(self.map_state.covisibility_graph)>0:
                if self.draw_covisibility:
                    gl.glLineWidth(1)
                    gl.glColor3f(0.0, 1.0, 0.0)
                    pypangolin.DrawLines(self.map_state.covisibility_graph,3)                                             
                    
            if len(self.map_state.spanning_tree)>0:
                if self.draw_spanning_tree:
                    gl.glLineWidth(1)
                    gl.glColor3f(0.0, 0.0, 1.0)
                    pypangolin.DrawLines(self.map_state.spanning_tree,3)              
                    
            if len(self.map_state.loops)>0:
                if self.draw_spanning_tree:
                    gl.glLineWidth(2)
                    gl.glColor3f(0.5, 0.0, 0.5)
                    pypangolin.DrawLines(self.map_state.loops,3)        
                    gl.glLineWidth(1)                                               
